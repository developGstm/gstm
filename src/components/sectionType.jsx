...
import modelServices from '../model/servicesModel'

function SectionType({type, title, clase}) {
  const [tickets, settickets] = useState([])
  useEffect(() => {
    Axios.get(`https://cms.gstmtravel.com/api/filterServiceGlobal/${type}`)
    .then(response => {
    ...
    return (
      <section className={clase}>
        <div className="mx-auto max-w-7xl p-6 lg:px-8">
          <h1 className="font-extrabold text-5xl mb-5 text-black lg:text-6xl" dangerouslySetInnerHTML={{__html: title}}>
          </h1>
